...
pub trait DcdtNftMarketplace {
	...
	fn auction_token(
		...
		let nft_nonce = self.call_value().dcdt_token_nonce();

		require!(
			self.call_value().dcdt_token_type() == DcdtTokenType::NonFungible,
			"Only Non-Fungible tokens can be auctioned"
		);
		require!(
			self.call_value().dcdt_value() == Self::BigUint::from(NFT_AMOUNT),
			"Token is not an NFT"
		);
		require!(
	...
	fn bid(&self, nft_type: TokenIdentifier, nft_nonce: u64) -> SCResult<()> {
		...
		let payment_token_nonce = self.call_value().dcdt_token_nonce();
	...
	fn end_auction(&self, nft_type: TokenIdentifier, nft_nonce: u64) -> SCResult<()> {
		...
		if auction.current_winner != Address::zero() {
			...
			let _ = self.send().direct_dcdt_nft_via_transfer_exec(
				&auction.current_winner,
				nft_type.as_dcdt_identifier(),
				nft_nonce,
